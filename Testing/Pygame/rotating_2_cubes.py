...
def main():
    ...
    # Define the vertices and edges for the first cube
    vertices1 = (
    ...
    cube1 = Physical(vertices=vertices1, edges=edges)

    # Define the vertices for the second cube, offsetting it by adding 3 to the x-coordinates
    vertices2 = tuple((x+3, y, z) for x, y, z in vertices1)

    cube2 = Physical(vertices=vertices2, edges=edges)
    ...
    def keys_handler(keys):
        for key in params:
            if keys[key]:
                cube1.rotate(*params[key])
                cube2.rotate(*params[key])  # Apply the same rotation to both cubes

    pygame.init()
    pygame.display.set_caption('Two Cubes -   q,w : X    a,s : Y    z,x : Z')

   # Correcting the MultiPaint class
    class MultiPaint:
        ...
        def __draw_shapes(self, thickness=4):  # Renamed to plural to indicate multiple shapes handling
        ...
        def __mainloop(self):
            while True:
                self.__handle_events()
                self.__screen.fill(BLACK)
                self.__draw_shapes()  # Correctly calls the new drawing method
                pygame.display.flip()
                self.__clock.tick(40)

    # Instantiate MultiPaint with both shapes
    MultiPaint([cube1, cube2], keys_handler)
