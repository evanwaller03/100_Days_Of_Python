...
def draw_snapshot(snapshot, layer):
    camera_pos = (-1, -10, 10)  # This should be dynamically determined based on your camera setup
    layer_height = -layer * 0.2
    spacing = 0.18  # Additional spacing between cubes
    for x in range(GRID_WIDTH):
        for y in range(GRID_HEIGHT):
            if snapshot[x, y] == 1:
                # Increase spacing by adjusting the multiplier and adding an explicit spacing term
                cube_x = ((x - GRID_WIDTH / 2) * 0.2) + (x * spacing)
                cube_y = ((y - GRID_HEIGHT / 2) * 0.2) + (y * spacing)
                cube_z = layer_height
                if is_cube_visible(cube_x, cube_y, cube_z, camera_pos):
                    distance = distance_to_center(x, y)
                    color = color_from_distance(distance)
                    glColor3f(*color)
                    draw_cube(cube_x, cube_y, cube_z, 0.1)
...
# def is_cube_visible(x, y, z, camera_pos, visibility_threshold=10):
#     """
#     Temporarily return True for debugging purposes.
#     """
#     return True
